def hexdump(bytes, bytes_per_line = 16, offset_size = 4):
  """
  ...
  offset_size : int
    Size of the location offset in the byte buffer in bytes. For example, if
    the buffer is less than 256 bytes, the offset need only be one byte long
    and can be set to 1. The offset is printed at the beginning of each line
